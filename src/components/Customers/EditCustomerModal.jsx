...
const EditCustomerModal = ({ isOpen, onClose, onSubmit, customer, darkMode }) => {
  ...
  useEffect(() => {
    if (customer) {
      setFormData({
        ...
        masterMobileNumber: customer.masterMobileNumber || ''
      });
    }
  }, [customer]);
        ...
        <div className={`inline-block align-bottom rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
          <div className={`px-4 pt-5 pb-4 sm:p-6 sm:pb-4 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-4">
                <label className={`block mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'} ${isRtl ? 'text-right' : 'text-left'}`}>
                  {t('translations:edit_customer_modal.company_name')}
                </label>
                <input
                  type="text"
                  name="companyName"
              ...
              <div className="mb-4">
                <label className={`block mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'} ${isRtl ? 'text-right' : 'text-left'}`}>
                  {t('translations:edit_customer_modal.mobile_number')}
                </label>
                <input
                  type="text"
                  name="mobileNumber"
              ...
              <div className="mb-4">
                <label className={`block mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'} ${isRtl ? 'text-right' : 'text-left'}`}>
                  {t('translations:edit_customer_modal.master_mobile_number')}
                </label>
                <input
                  type="text"
              ...
              <div className="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                ...
                >
                  {t('translations:edit_customer_modal.update_button')}
                </button>
                <button
                  type="button"
                  onClick={onClose}
                ...
                >
                  {t('translations:edit_customer_modal.cancel_button')}
                </button>
              </div>
            </form>
          </div>
